...
def imread_unicode(filename):
    """读取图片，支持中文/Unicode 路径。

    OpenCV 的 cv2.imread 在 Windows 下无法处理非 ASCII 路径（会静默返回
    None）。这里改用 numpy 读取原始字节 + cv2.imdecode 来规避该问题。
    """
...
def imwrite_unicode(filename, img):
    """写入图片，支持中文/Unicode 路径。

    用 cv2.imencode 编码后通过 numpy 的 tofile 写出，规避 cv2.imwrite 对
    非 ASCII 路径的限制。返回是否成功。
    """
...
class WaterMarkCore:
    ...
    def embed(self, filename):
        ...
        if not imwrite_unicode(filename, embed_img):
            raise IOError("image file '{filename}' write failed".format(filename=filename))
        return embed_img
